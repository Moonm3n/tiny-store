#!/bin/bash
cd /home/zhanyueyang/tiny-store

rm -rf build # 清除之前编译的结果
cd build
cmake ..
make -j 32          # 重新编译
if [ -f log ]; then # log文件已存在则删除
  rm -r log
fi
if [ -f error ]; then # error文件已存在则删除
  rm -r error
fi
count=5000 # 13小时测试，日志每10s记录一次，因此循环5000次
echo '测试开始时间：'$(date) >>log
./test >>log 2>>error & # 运行B+树并发读写，将错误信息保存到error中
