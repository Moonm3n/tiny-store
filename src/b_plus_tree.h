...
class BPlusTree  // B+树
{
 public:
  ...
  ~BPlusTree();
  void set_order(int num) { this->bpt_order = num; }
  int get_order() { return this->bpt_order; }
  ...
  void destroy_bpt();                        // 销毁B+树
  void save_bpt(const std::string& filename);       // 将B+树保存到磁盘中
  void read_bpt(const std::string& filename);       // 从磁盘中恢复B+树
  void print_leaves();                       // 打印叶子节点
  void print_tree();                         // 打印整个B+树
  // 并发操作
  void set_buffersize(int bsize) { this->buffer_size = bsize; }
 ...
 private:
  ...
  static void insert_directly(BPT_Node* insert_pos, int key, void* value);  // 直接向指向的节点中插入数据
  ...
  void add_command(const MyCommand& com);           // 生产者线程：向缓存区中添加task
